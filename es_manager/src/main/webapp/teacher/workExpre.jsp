 ...
 $('#flexigridData').datagrid({
  			  ...
  			  columns: [[
  			           { field: 'no', title: '<label for="no">编号</label>'}
  						, { field: 'positions', title: '<label for="positions">职位</label>', width: 226 }
    						, { field: 'workerNo', title: '<label for="workerNo">工号</label>', width: 226
  						},{field: 'compony', title: '<label for="compony">工作单位</label>', width: 226},
  						{ field: 'begintime', title: '<label for="begintime">开始时间</label>', width: 226 ,formatter:function(date){
  							var dt = new Date(date).format("yyyy-MM-dd"); 
  							return dt.toString();
  						}
  						},
  						{ field: 'endtime', title: '<label for="endtime">结束时间</label>', width: 226 ,formatter:function(date){
  							var dt = new Date(date).format("yyyy-MM-dd"); 
  							return dt.toString();
  						}},
  						{field: 'des', title: '<label for="des">描述</label>', width: 226}
        ...
        function flexiQuery() {
        	...
        	if(tno==""){
        	 ...
        	 $('#flexigridData').datagrid({
                 ...
                 collapsible: true,
                 singleSelect: true,
                 url: '/es_manager/teachermanager/getWorkExpre', //获取数据的url
                 queryParams:{workerNo:tno},
                 sortName: 'Id',
                 sortOrder: 'desc',
                 idField: 'Id',
  				toolbar:'#search',
        ...
        function flexiCreate() {
            window.location.href = "/es_manager/teacher/addExpre.jsp";
            return false;
        }
        //导航到修改的按钮
        ...
        function flexiDelete() {
            ...
            $.messager.confirm('操作提示', "确认删除这 " + rows.length + " 项吗？", function (r) {
                if (r) {
                    $.post("/es_manager/teachermanager/deleteExpre", { expreNo:no }, function (res) {
                        if (res.status == "200") {
                            //移除删除的数据
