...
<script type="text/javascript" language="javascript">
 ...
 $('#flexigridData').datagrid({
                 ...
                 collapsible: true,
                 singleSelect: true,
                 url: '/es_manager/examArrange/getArrangeDetail', //获取数据的url
                 sortName: 'Id',
                 queryParams:{condition:query},
                 sortOrder: 'desc',
                 idField: 'Id',
  				toolbar:'#search',
  			  columns: [[
  			         ...
  			         { field: 'SNO', title: '<label for="SNO">学号</label>'}
  						...
  						{ field: 'classno', title: '<label for="classno">教室</label>', width: 100 },
  						 { field: 'SEAT_NUMBER', title: '<label for="SEAT_NUMBER">座号</label>'},
        ...
        function flexiCreate() {
            window.location.href = "/es_manager/exam/addDetail.jsp?examno=${arrangeNo}";
            return false;
        }
        
        
        function flexiCreateFast(){
        	 $.post("/es_manager/examArrange/insertExamArrange",{examno:${arrangeNo}},function(result){
        ...
        function flexiModify() {
            ...
            if (arr.length == 1) {
                window.location.href = "/es_manager/examArrange/toUpdateDetail?no="+arr[0].NO;
            } else {
        ...
        function flexiDelete() {
            ...
            $.messager.confirm('操作提示', "确认删除这 " + rows.length + " 项吗？", function (r) {
                if (r) {
                    $.post("/es_manager/examArrange/deleteExamArrangeDetail", {no:no}, function (res) {
                        if (res.status == "200") {
                            //移除删除的数据
                            $("#flexigridData").datagrid("reload");
		...
		<table border="0" cellpadding="0" cellspacing="0">
			<tr>
				...
				<td><a href="#" onclick="flexiCreate();"
					...
					<a href="#" onclick="flexiCreateFast();"
					class="easyui-linkbutton" data-options="iconCls:'icon-add'">快速添加</a>
