...
<script type="text/javascript" language="javascript">
 ...
 $('#flexigridData').datagrid({
                 ...
                 collapsible: true,
                 singleSelect: true,
                 url: '/es_manager/bookmanager/getBook', //获取数据的url
                 sortName: 'Id',
                 sortOrder: 'desc',
                 idField: 'Id',
  				toolbar:'#search',
  			  columns: [[
  			           { field: 'book_no', title: '<label for="book_no">书号</label>'}
  						, { field: 'book_name', title: '<label for="bookName">书名</label>', width: 226 }
  						, { field: 'press', title: '<label for="press">出版社</label>', width: 100 }
  						, { field: 'author', title: '<label for="author">作者</label>', width: 226
  						}
  						, { field: 'price', title: '<label for="price">价格</label>', width: 100 }
  	                ]],
                 pagination: true,
                 rownumbers: true
        ...
        function flexiCreate() {
            window.location.href = "/es_manager/book/addBook.jsp";
            return false;
        }
        //导航到修改的按钮
        function flexiModify() {
            ...
            if (arr.length == 1) {
                window.location.href = "/es_manager/bookmanager/toModifyBook?no="+arr[0].book_no;
            } else {
        ...
        function flexiDelete() {
            ...
            var no = rows[0].book_no;
            $.messager.confirm('操作提示', "确认删除这 " + rows.length + " 项吗？", function (r) {
                if (r) {
                    $.post("/es_manager/bookmanager/deleteBook", {bookNo:no}, function (res) {
                        if (res.status == "200") {
                            //移除删除的数据
			...
			<tr>
				<td>
					<form id="queryCondition">
						书号： <input name="bookNo" id="bookNo" type="text" style="width: 80px">
						&nbsp;&nbsp;书名： <input name="bookname" id="bookname" type="text"
							style="width: 80px"> 
						出版社： <input name="press" id="press" type="text" style="width: 80px">
