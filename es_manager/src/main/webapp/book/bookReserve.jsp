...
<script type="text/javascript" language="javascript">
 ...
 $('#flexigridData').datagrid({
                 ...
                 collapsible: true,
                 singleSelect: true,
                 url: '/es_manager/bookReserve/getBookReserve', //获取数据的url
                 sortName: 'Id',
                 sortOrder: 'desc',
                 idField: 'Id',
  				toolbar:'#search',
  			  columns: [[
  			           { field: 'no', title: '<label for="no">序号</label>'}
    						, { field: 'bookname', title: '<label for="bookname">书名</label>', width: 226
  						...
  						{field: 'sname', title: '<label for="sname">姓名</label>', width: 226},
  						{field: 'classname', title: '<label for="classname">班级</label>', width: 226},
  						{field: 'college', title: '<label for="college">学院</label>', width: 226},
  						{field: 'bookNo', title: '<label for="bookNo">书号</label>', width: 226},
  						{field: 'paystatus', title: '<label for="paystatus">缴费状态</label>', width: 226},
  						{field: 'payable', title: '<label for="payable">应缴</label>', width: 226},
  						{field: 'payin', title: '<label for="payin">实缴</label>', width: 226}
        ...
        function flexiCreate() {
            window.location.href = "/es_manager/book/addBookBuy.jsp";
            return false;
        }
        //导航到修改的按钮
        function flexiModify() {
            ...
            if (arr.length == 1) {
                window.location.href = "/es_manager/teachermanager/toModifyExpre?no="+arr[0].no;
            } else {
        ...
        function flexiDelete() {
            ...
            $.messager.confirm('操作提示', "确认删除这 " + rows.length + " 项吗？", function (r) {
                if (r) {
                    $.post("/es_manager/bookOrder/deleteBookOrder", { bookOrderNo:no }, function (res) {
                        if (res.status == "200") {
                            //移除删除的数据
			...
			<tr>
				<td>
					<form id="queryCondition">
						书名： <input name="bookname" id="bookname" type="text" style="width: 80px">
						学院：<select id="college" name="college">
							<option value="">全部</option>
						</select>
						班级：<input name="classno" id="classno" type="text" style="width: 80px">
						学年： <select id="yeartime" name="yeartime"></select>
						学期：<select id="termtime" name="termtime">
								  <option value="1">1</option>
								  <option value="2">2</option>
						      </select>
						 缴费状态：<select id="paystatus" name="paystatus">
