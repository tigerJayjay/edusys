...
<script type="text/javascript" language="javascript">
 ...
 $('#flexigridData').datagrid({
                 ...
                 collapsible: true,
                 singleSelect: true,
                 url: '/es_manager/bookPayInfo/getBooPayInfo', //获取数据的url
                 sortName: 'Id',
                 sortOrder: 'desc',
                 idField: 'Id',
  				toolbar:'#search',
				...
				{ field: 'no', title: '<label for="no">编号</label>'}
					...
					, { field: 'bookname', title: '<label for="bookname">书名</label>', width: 100 },
					{ field: 'price', title: '<label for="price">单价</label>', width: 100 },
					{ field: 'amount', title: '<label for="amount">实缴</label>', width: 100 },
					{ field: 'yeartime', title: '<label for="yeartime">学年</label>', width: 100 },
					{ field: 'termtime', title: '<label for="bookname">学期</label>', width: 100 }
  	                ]],
                 pagination: true,
                 rownumbers: true
        ...
        function flexiCreate() {
            window.location.href = "/es_manager/book/addBookPay.jsp";
            return false;
        }
        //导航到修改的按钮
        function flexiModify() {
            ...
            if (arr.length == 1) {
                window.location.href = "/es_manager/bookPayInfo/toModifyBookPay?no="+arr[0].no;
            } else {
        ...
        function flexiDelete() {
            ...
            $.messager.confirm('操作提示', "确认删除这 " + rows.length + " 项吗？", function (r) {
                if (r) {
                    $.post("/es_manager/bookPayInfo/deleteBooPayInfo", {bookPayInfoNo:no}, function (res) {
                        if (res.status == "200") {
                            //移除删除的数据
